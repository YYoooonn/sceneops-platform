...
from sceneops_db.postgres import PostgresEpisodeRunRepository
from sceneops_db.repositories import EpisodeRunRecord


class EpisodeRunStore:
    def __init__(self, session: AsyncSession) -> None:
        self._repo = PostgresEpisodeRunRepository(session)
    ...
    async def save(self, run: EpisodeRunRecord) -> EpisodeRunRecord:
        return await self._repo.update(run)

    async def upsert(self, run: EpisodeRunRecord) -> EpisodeRunRecord:
